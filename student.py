...
# ============ variables ============
    ...
    # Department
        ...
        dep_combo = ttk.Combobox    (current_course_frame,textvariable=self.var_dep, font=("times new roman", 12, "bold"), state="readonly") # to create a combo box to select the department
        dep_combo["values"] = ("Select Department", "Computer Science", "Information Technology", "Electronics", "Mechanical", "Civil") 
        dep_combo.current(0) # to set the default value of the combo box
        dep_combo.grid(row=0, column=1, padx=10, pady=5) # to place the combo box in the tkinter window
    ...
    # Student ID
        ...
        studentID_entry = ttk.Entry(class_student_frame, textvariable=self.var_std_id, font=("times new roman", 12, "bold"))
    ...
    # Buttons Frame
        ...
        take_photo_btn = Button(btn_frame1,command=self.generate_dataset, text="Take Photo Sample", font=("times new roman", 12, "bold"), bg="blue", fg="white", width=35)
        take_photo_btn.grid(row=0, column=0, padx=10, pady=5)
        
        update_photo_btn = Button(btn_frame1, text="Update Photo Sample", font=("times new roman", 12, "bold"), bg="blue", fg="white", width=35)
    ...
    # Right label frame
        ...
        search_combo = ttk.Combobox(search_frame, font=("times new roman", 13, "bold"), width=13, state="readonly")
        search_combo["values"] = ("Select","Roll_NO","Student_ID", "Name", "Email")
     ...
     # =========Table Frame=========
        ...
        self.student_table = ttk.Treeview(table_frame, column=("dep", "course", "year", "sem", "id", "name", "div", "roll_no", "gender", "email", "phone", "address", "teacher","photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                while True:
                    ...
                    if cv2.waitKey(1) == 13 or int(img_id) == 100: #13 is the ASCII code for Enter key
                        break
                cap.release()
